...
const parse = (expression) => {
    ...
    const matches = expression.replace(/\s+/g, ' ').match(PATTERN);
    if (matches === null) {
        return null;
    }
...
const parseOperation = (expression) => {
    ...
    const operations = querystring.parse(expression.replace(/\s+/g, '').replace(/，/g, ',').replace('+', '_|_'), ',', '=');
    for (const key in operations) {
        operations[key] = parseValue(operations[key].replace('_|_', '+'));
    }
