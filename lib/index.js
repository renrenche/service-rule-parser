...
const parse = (expression) => {
    if (!expression || typeof expression !== 'string') {
        debug(`${pkgInfo.name}: expect 'expression' param to be an non-empty string`);
        return null;
    }
    ...
    if (matches === null) {
        debug(`${pkgInfo.name}: 'expression' must be in 'SET xxx=xxx WHERE yyy=yyy' format, expression: ${expression}`);
        return null;
    }
