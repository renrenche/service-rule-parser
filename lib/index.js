...
const parse = (expression) => {
    if (typeof expression !== 'string') {
        console.error(`${pkgInfo.name}: expect 'expression' arg to be a string`);
        return null;
    }

    const sanitized = expression.replace(/\s+/g, ' ').replace('，', ',');
    const matches = sanitized.match(PATTERN);
    if (matches === null) {
        console.error(`${pkgInfo.name}: 'expression' must be in 'SET xxx=xxx WHERE yyy=yyy' format`);
        return null;
    }
    ...
    try {
        ...
        console.error(`${pkgInfo.name}: rule parse failed: ${e.message}`);
